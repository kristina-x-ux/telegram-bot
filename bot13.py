...
MENU = {
    ...
    "6": {
        ...
        "items": {
            ...
            "Луковые кольца": 250,
            "Креветки к пиву": 450,
            "Креветки в медово-горчичном соусе": 450,
        },
    },
}
...
CHUKA_NAME = "Салат Чука"
CHUKA_PRICE = 250
PUBLIC_POST_HOUR = 15
PUBLIC_POST_MINUTE = 30
...
def pg_get_conn():
    return psycopg2.connect(
        ...
        sslmode="require",
        connect_timeout=10,
        application_name="kakmakdak_bot",
    )
...
def get_user_orders(user_id, limit=5):
    visible_statuses = ("accepted", "on_the_way")

    if USE_POSTGRES:
        with pg_get_conn() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute("""
                    SELECT order_id_text, total, payment, address, locality, comment, cart_json, created_at, status
                    FROM (
                        SELECT
                            o.order_id_text,
                            o.total,
                            ...
                            o.cart_json,
                            o.created_at,
                            COALESCE(os.status, 'new') AS status
                        FROM orders o
                        LEFT JOIN order_statuses os
                            ON os.order_id_text = o.order_id_text
                        WHERE o.user_id = %s
                          AND COALESCE(os.status, 'new') = ANY(%s)
                        ORDER BY o.created_at DESC
                        LIMIT %s
                    ) recent_orders
                    ORDER BY created_at ASC
                """, (user_id, list(visible_statuses), limit))
                rows = cur.fetchall()
    ...
    accepted_history = [
        ...
        if order.get("status") in visible_statuses
    ]
...
def set_order_status(order_id, status, admin_message_chat_id=None, admin_message_id=None):
    if USE_POSTGRES:
        with pg_get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    INSERT INTO order_statuses (order_id_text, status, admin_message_chat_id, admin_message_id)
                    VALUES (%s, %s, %s, %s)
                    ...
                    DO UPDATE SET
                        ...
                        updated_at = NOW()
                """, (order_id, status, admin_message_chat_id, admin_message_id))
        return
...
def build_admin_status_keyboard(order_id, client_chat_id, phone, status):
    ...
    kb.row(
        types.InlineKeyboardButton("✅ Принят" + (" •" if status == "accepted" else ""), callback_data=f"admin_accept|{order_id}|{client_chat_id}"),
        types.InlineKeyboardButton("🚚 Уже в пути" + (" •" if status == "on_the_way" else ""), callback_data=f"admin_way|{order_id}|{client_chat_id}")
    )
    kb.add(types.InlineKeyboardButton("❌ Отменить заказ" + (" •" if status == "cancelled" else ""), callback_data=f"admin_cancel|{order_id}|{client_chat_id}"))
    return kb


def build_admin_order_text(base_text, status):
    status_map = {
        "new": "🕓 Новый",
        "accepted": "✅ Принят",
        "on_the_way": "🚚 Уже в пути",
        "cancelled": "❌ Отменён",
    }
...
def update_admin_order_message(call, order_id, client_chat_id, phone, status):
    current_text = call.message.text or f"📥 НОВЫЙ ЗАКАЗ\n{order_id}"
    base_text = current_text.split("\n\nСтатус:")[0]
    new_text = build_admin_order_text(base_text, status)
    ...
    if ok:
        set_order_status(order_id, status, call.message.chat.id, call.message.message_id)
    return ok
...
def format_order_history_item(order):
    ...
    status_text = f"\n📌 Статус: {status_map.get(status, status)}" if status in status_map else ""
...
def build_yandex_maps_link_from_text(address_text, locality="Не определён"):
    query_parts = []
    if locality and locality != "Не определён":
        query_parts.append(locality)
    query_parts.append(address_text)
...
def public_post_scheduler():
    # При запуске отправляем один раз в сутки отдельным ключом,
    # чтобы пост мог прийти и при старте, и потом ещё в 15:30.
    send_public_post("last_public_startup_post_date")
...
# ОБЩАЯ ОТМЕНА / СБРОС
# =========================
@bot.message_handler(func=lambda m: m.chat.type == "private" and m.text == "❌ Отмена")
def cancel_current_flow(message):
    apply_cancel_last_action(message.chat.id, message.from_user.id)
...
def callbacks(call):
    ...
    if data.startswith("admin_accept|"):
        ...
        safe_send_message(
            call.message.chat.id,
            "✅ Статус заказа «Принят» отправлен клиенту."
        )
        return
    ...
    if data.startswith("admin_way|"):
        ...
        safe_send_message(
            call.message.chat.id,
            "🚚 Статус заказа «Уже в пути» отправлен клиенту."
        )
        return
    ...
    if data.startswith("admin_cancel|"):
        ...
        safe_send_message(
            call.message.chat.id,
            "❌ Статус заказа «Отменён» отправлен клиенту."
        )
        return
...
def process_manual_address(message):
    ...
    user_data["need_manual_clarification"] = False
    if user_data.get("locality", "Не определён") == "Не определён":
        user_data["locality"] = detect_locality_from_text(text)
    user_data["map_link"] = build_yandex_maps_link_from_text(text, user_data.get("locality", "Не определён"))
    save_user_data(user_id, user_data)
...
def handle_comment_actions_unused(call):
    user_id = call.from_user.id

    if call.data == "edit_comment":
        set_user_state(user_id, STATE_WAIT_COMMENT_OR_SKIP)
        bot.answer_callback_query(call.id, "Можешь изменить комментарий")
        safe_send_message(call.message.chat.id, "✏️ Введите комментарий заново или нажмите Пропустить:", reply_markup=get_comment_skip_keyboard())
        return

    if call.data == "comment_next":
        set_user_state(user_id, STATE_WAIT_CONFIRM)
        bot.answer_callback_query(call.id)
        safe_send_message(
            call.message.chat.id,
...
def finish_order(message, phone, user_id):
    ...
    map_link_text_admin = f"\n🗺 Яндекс.Карты: {map_link}" if map_link else ""
    ...
    set_order_status(order_id, "new", ADMIN_CHAT_ID, sent_to_admin.message_id)
...
def run_bot():
    ...
    logger.info("Бот запущен. Режим хранения: PostgreSQL%s", " | Railway" if RAILWAY_ENVIRONMENT else "")
