...
RESTAURANT_ROUTE_ORIGIN = "Ялта, Новый переулок 2"
...
def format_order_history_item(order):
    ...
    locality = order.get("locality", "Не определён")
    locality_text = f"📍 Населённый пункт: {locality}\n" if locality and locality != "Не определён" else ""
...
def extract_clean_address_for_maps(address_text):
    ...
    cut_index = len(value)
    for pattern in explicit_detail_patterns:
        match = re.search(pattern, lowered, flags=re.IGNORECASE)
        if match:
            cut_index = min(cut_index, match.start())

    base_value = value[:cut_index].strip(" ,.;") if cut_index < len(value) else value

    street_house_match = re.match(
        r"^(.+?\s+\d+[а-яa-z]?"
        r"(?:/[0-9а-яa-z-]+)?"
        r"(?:\s*(?:к|корпус|стр|строение|лит|литера)\.?\s*[0-9а-яa-z-]+)?)"
        r"(?:\b|$)",
        base_value,
        flags=re.IGNORECASE,
    )
    if street_house_match:
        cleaned = street_house_match.group(1).strip(" ,.;")
        return re.sub(r"\s+", " ", cleaned)

    return re.sub(r"\s+", " ", base_value).strip(" ,.;")

def split_address_details(address_text):
    original = (address_text or "").strip()
    if not original:
        return "", ""

    clean_address = extract_clean_address_for_maps(original)
    clean_address = (clean_address or original).strip(" ,.;")

    details = ""
    if clean_address:
        pattern = re.compile(r"^" + re.escape(clean_address) + r"(?:[\s,.;:-]+)?", flags=re.IGNORECASE)
        details = pattern.sub("", original, count=1).strip(" ,.;:-")

    return clean_address or original, details
...
def build_yandex_maps_link_from_text(address_text, locality="Не определён"):
    ...
    for candidate_locality in build_locality_search_order(explicit_locality if explicit_locality != "Не определён" else None):
        ...
        if match and match.get("lat") and match.get("lon"):
            lat = match["lat"]
            lon = match["lon"]
            return f"https://yandex.ru/maps/?pt={lon},{lat}&z=17&l=map"

    locality_hint = explicit_locality if explicit_locality != "Не определён" else "Ялта"
    fallback_query = f"{locality_hint}, Крым, {clean_address_text or address_text}"
    query = quote(fallback_query)
    return f"https://yandex.ru/maps/?text={query}"



def build_yandex_route_link_to_address(address_text, locality="Не определён", lat=None, lon=None):
    """
    Строит маршрутную ссылку Яндекс.Карт от ресторана до клиента.
    Важно: расстояние и время считает само приложение Яндекс.Карт при открытии ссылки.
    """
    origin = RESTAURANT_ROUTE_ORIGIN

    if lat is not None and lon is not None:
        # В rtext Яндекс.Карт координаты указываются как широта,долгота.
        destination = f"{float(lat)},{float(lon)}"
    else:
        clean_address_text = extract_clean_address_for_maps(address_text)
        explicit_locality = locality if locality in LOCALITIES else detect_locality_from_text(clean_address_text)
        locality_hint = explicit_locality if explicit_locality != "Не определён" else "Ялта"
        destination = f"{locality_hint}, Крым, {clean_address_text or address_text}"
...
def show_or_update_preview(chat_id, user_id, last_added_text="—"):
    ...
    if total_qty == 0:
        user_data.pop("last_preview_last_added", None)
        save_user_data(user_id, user_data)
        preview_text = "🛒 Предкорзина пуста."
    else:
        if last_added_text and last_added_text != "—":
            user_data["last_preview_last_added"] = last_added_text
            save_user_data(user_id, user_data)
        else:
            last_added_text = user_data.get("last_preview_last_added", "—")
...
def keep_preview_at_bottom(chat_id, user_id):
    total_qty, _ = get_cart_total(user_id)
    if total_qty <= 0:
        clear_preview(chat_id, user_id)
        return
...
def handle_location(message):
    ...
    user_data["route_lat"] = lat
    user_data["route_lon"] = lon
    user_data["route_link"] = build_yandex_route_link_to_address(address, locality, lat, lon)

    locality_text = f"📍 Населённый пункт: {locality}\n" if locality and locality != "Не определён" else ""

    if needs_manual_address:
        user_data["address"] = address
        user_data["address_full"] = address
        user_data["address_for_map"] = address
...
def process_manual_address(message):
    ...
    previous_locality = user_data.get("locality", "Не определён")
    clean_address_text, address_details = split_address_details(text)
    explicit_locality = detect_locality_from_text(text)
    search_hint_locality = explicit_locality if explicit_locality != "Не определён" else (previous_locality if previous_locality != "Не определён" else None)
    search_order = build_locality_search_order(search_hint_locality)

    detected_locality = explicit_locality if explicit_locality != "Не определён" else (previous_locality if previous_locality != "Не определён" else "Не определён")
    map_link = ""
    route_lat = None
    route_lon = None
    ...
    for locality_name in search_order:
        ...
        if match:
            ...
            route_lat = match.get("lat")
            route_lon = match.get("lon")
            break

    if not map_link:
        map_link = build_yandex_maps_link_from_text(clean_address_text, detected_locality)

    route_link = build_yandex_route_link_to_address(clean_address_text, detected_locality, route_lat, route_lon)

    user_data["address"] = text
    user_data["address_full"] = text
    user_data["address_for_map"] = clean_address_text
    user_data["clean_address"] = clean_address_text
    user_data["address_details"] = address_details
    user_data["need_manual_clarification"] = False
    user_data["locality"] = detected_locality
    user_data["map_link"] = map_link
    user_data["route_link"] = route_link
    if route_lat is not None and route_lon is not None:
        user_data["route_lat"] = route_lat
        user_data["route_lon"] = route_lon
    else:
        user_data.pop("route_lat", None)
        user_data.pop("route_lon", None)
    save_user_data(user_id, user_data)

    set_user_state(user_id, STATE_WAIT_PHONE)
    locality_text = f"📍 Населённый пункт: {detected_locality}\n" if detected_locality and detected_locality != "Не определён" else ""
    safe_send_message(
        message.chat.id,
        f"{locality_text}"
        f"🏠 Адрес: {text}\n\n"
...
def finish_order(message, phone, user_id):
    ...
    address_full = user_data.get("address_full", address)
    address_for_map = user_data.get("address_for_map") or user_data.get("clean_address") or extract_clean_address_for_maps(address)
    clean_address = user_data.get("clean_address") or address_for_map or address
    address_details = user_data.get("address_details", "")
    locality = user_data.get("locality", "Не определён")
    ...
    if not map_link and address_for_map:
        map_link = build_yandex_maps_link_from_text(address_for_map, locality)

    route_link = user_data.get("route_link", "")
    route_lat = user_data.get("route_lat")
    route_lon = user_data.get("route_lon")
    if not route_link and address_for_map:
        route_link = build_yandex_route_link_to_address(address_for_map, locality, route_lat, route_lon)

    map_link_text_admin = ""
    if route_link:
        map_link_text_admin = (
            f"\n🗺 Маршрут от Новый переулок 2: {route_link}"
            "\n📏 Расстояние по дороге: откройте маршрут в Яндекс.Картах"
        )
    elif map_link:
        map_link_text_admin = f"\n🗺 Маршрут: {map_link}"

    locality_text_user = f"📍 Населённый пункт: {locality}\n" if locality and locality != "Не определён" else ""
    details_text_admin = f"\n📝 Детали адреса: {address_details}" if address_details else ""
    ...
    admin_text = (
        ...
        f"🏠 Адрес: {clean_address or address_full}"
        f"{details_text_admin}"
