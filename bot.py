...
def keep_preview_at_bottom(chat_id, user_id):
    ...
    old_preview_message_id = get_preview_message_id(user_id)
    msg = safe_send_message(chat_id, preview_text, reply_markup=get_preview_keyboard())
    if msg:
        set_preview_message_id(user_id, msg.message_id)
        if old_preview_message_id:
            safe_delete_message(chat_id, old_preview_message_id)
...
def show_menu(message):
    ...
    keep_preview_at_bottom(message.chat.id, message.from_user.id)
...
def repeat_last_order(message):
    ...
    keep_preview_at_bottom(message.chat.id, user_id)
...
def callbacks(call):
    ...
    if data == "back_categories":
        ...
        keep_preview_at_bottom(call.message.chat.id, user_id)
        return
...
def process_manual_address(message):
    ...
    clean_address_text = extract_clean_address_for_maps(text) or text
...
def finish_order(message, phone, user_id):
    ...
    address_for_map = user_data.get("address_for_map", address)
