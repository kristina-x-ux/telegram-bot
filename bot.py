...
def callbacks(call):
    ...
    if data.startswith("admin_accept|"):
        ...
        set_order_status(order_id, "accepted", call.message.chat.id, call.message.message_id)
        mark_order_accepted_for_history(int(client_chat_id), order_id)
        update_admin_order_message(call, order_id, int(client_chat_id), phone, "accepted")
        safe_send_message(
            int(client_chat_id),
            f"✅ Заказ принят\n{order_id}",
            reply_markup=get_main_keyboard()
    ...
    if data.startswith("admin_way|"):
        _, order_id, client_chat_id = data.split("|")
        phone = extract_phone_from_text(call.message.text or "")
        set_order_status(order_id, "on_the_way", call.message.chat.id, call.message.message_id)
        update_admin_order_message(call, order_id, int(client_chat_id), phone, "on_the_way")
        safe_send_message(
            int(client_chat_id),
            f"🚚 Заказ уже в пути\n{order_id}",
            reply_markup=get_main_keyboard()
    ...
    if data.startswith("admin_cancel|"):
        _, order_id, client_chat_id = data.split("|")
        phone = extract_phone_from_text(call.message.text or "")
        set_order_status(order_id, "cancelled", call.message.chat.id, call.message.message_id)
        update_admin_order_message(call, order_id, int(client_chat_id), phone, "cancelled")
        safe_send_message(
            int(client_chat_id),
