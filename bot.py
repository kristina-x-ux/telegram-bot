...
def get_order_meta(order_id):
    ...
    return storage.get("admin_order_meta", {}).get(order_id, {}).copy()
...
def mark_order_stale_if_needed(order_id, chat_id=None, message_id=None, client_chat_id=None):
    ...
    try:
        temp_message = bot.forward_message(admin_chat_id, admin_chat_id, admin_message_id)
        current_text = temp_message.text or ""
        try:
            bot.delete_message(admin_chat_id, temp_message.message_id)
        except Exception:
            pass
    except Exception:
        current_text = ""
    ...
    ok = safe_edit_message_text(
        ...
        reply_markup=build_admin_status_keyboard(order_id, client_chat_id or admin_chat_id, extract_phone_from_text(current_text), "stale"),
        disable_web_page_preview=True
    )
    if ok:
        set_order_status(order_id, "stale", admin_chat_id, admin_message_id)
    return ok
...
def build_yandex_maps_link_from_text(address_text, locality="Не определён"):
    explicit_locality = locality if locality in LOCALITIES else detect_locality_from_text(address_text)

    for candidate_locality in build_locality_search_order(explicit_locality if explicit_locality != "Не определён" else None):
        match = geocode_address_with_locality(address_text, candidate_locality)
        if match:
            return match["map_link"]

    fallback_query = f"Ялта, Крым, {address_text}"
...
def process_manual_address(message):
    ...
    for locality_name in search_order:
        match = geocode_address_with_locality(text, locality_name)
        if match:
    ...
    if not map_link:
        map_link = build_yandex_maps_link_from_text(text, detected_locality)
